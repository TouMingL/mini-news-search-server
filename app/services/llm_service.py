...
- 若提供的新闻中无与用户所问条件直接相关的内容，必须明确说明未检索到该主体的相关信息（需点明具体主体，如「未检索到森林狼相关信息」），不得用其他条件的同品类数据作答。
...
6. 若未检索到用户问题相关信息，仅输出一句简短说明并点明具体主体（如「未检索到森林狼相关信息」），无其他铺垫；若赛况数据引擎标注了具体缺失原因（「注意：...」），直接转述该原因。
